...
### September19 — private release candidate and package preparation

Owner authorized Apache-2.0 for World-owned code, release-candidate packages, private GitHub delivery and Cloudflare distribution preparation. **Repository visibility stays PRIVATE until hands-on package testing, bug fixes and separate explicit public approval.** See [release scope](docs/RELEASE_PREPARATION.md), [privacy findings](docs/RELEASE_PRIVACY_REVIEW.md), [product privacy](PRIVACY.md) and [SignPath preparation](docs/SIGNPATH_APPLICATION.md).

Website https://agentintersect.com/ and Field Guide https://guide.agentintersect.com/getting-started/ are deployed. Owner confirmed the touch-device poster rendering correction on an iPhone16 Pro. Website source is independently private and CI-green.

Candidate0.15.0-rc.1 adds the local packaged launcher, Linux x64 archive, Windows x64 installer recipe, locked production pruning/runtime checksums, Apache license/notices, genuine README screenshots/install/uninstall guidance and a private-only package CI workflow. The launcher remains loopback/browser-based and does not configure agents. Native Windows install/start/index/WebSocket/quit/uninstall smoke passes after correcting a POSIX-only internal temporary-root assumption. Linux extracted startup and shutdown pass. Local package attempts remain development artifacts until exact committed-source CI packages replace them.

Local gate: formatting, lint, types, architecture,1341 tests, production build, existing smoke, package lifecycle smoke and deployment-metadata regression pass. The first frozen-install attempt exposed pnpm hard-link mutation of four workspace manifests; they were restored and the builder now atomically replaces deployment metadata and verifies source-manifest/lock bytes remain unchanged. The corrected end-to-end package attempt passes that invariant.

External blockers: R2 access returns403 with current credential; SignPath eligibility/maintainer fields/consent are unresolved; protected AGENTS.md path-scrub approval timed out. Current-tree path cleanup does not sanitize private history. No signing, public-repository conversion, provider changes or retained-review cleanup is claimed. Exact commit/CI/release receipts live externally to avoid recursive status commits.

PR18/main9843fb0 is already merged and green; the older pending CI narrative below is historical and must not reopen that work.

### Historical September18 — proportional CI after merged PR17
...
Separate normal-UI review: **http://127.0.0.1:45379/**, kit `/home/user/.hermes/runs/aiw-pr17-city-v2-review-20260918`. Original TEST45377 and all older lanes remain untouched. This cumulative revision builds on PR17 baseline `e58a4b2`; its final exact-head delivery gates are recorded separately. Aaron (`1550516167171379241`) reports the effects working as intended and the Graphics menu “perfect,” with one remaining visual defect: the base fog appears as three separated layers cutting across models from most angles. Preserve that earlier qualified verdict; the later corrected-fog image approval is recorded above. Screenshot: `/home/user/.hermes/cache/images/img_41ad2bb3e6d6.png`. The fog-only correction is now implemented and technically verified as described below; the retained preview has not been replaced. Subsequent exact-new-head CI remains pending.
...
Corrected fog is **image-approved for delivery**; no new native interactive test is claimed. TEST45379/current user state and older lanes are untouched; its read-only health/served-index check passed. Automated proof servers are stopped. Evidence `/home/user/.hermes/runs/aiw-fog-20260918/`; normal candidate `/tmp/aiw-fog-20260918/web`. Commit/push and green-gated merge are now authorized. No separate manual retest is required; retained-runtime replacement or cleanup remains unapproved.
...
**Retained TEST: http://127.0.0.1:45353/**, backend43953/native Edge49353, `aiw-arrangement-review-20260915.service`. Preserve its current user setup/history and served artifact; the final footer patch is not activated there and no additional operator retest is required. Kit `/home/user/.hermes/runs/aiw-arrangement-review-20260915/README.md`. Prior TEST45351 browser was closed by Aaron; services/state/profile and original shared web dist remain retained. Seven inherited residuals are excluded from delivery.
...
Aaron subsequently requested closing old TEST45345, opening a fresh test, leaving it running, then prep end session. Preserve old profile/history on disk; do not import old setup/roster into the new lane. Current launch/readiness/stop details are recorded in `/home/user/.hermes/runs/aiw-project-overview-test-20260915/README.md` and the latest Obsidian handoff. This supersedes old TEST-retention restrictions below for this authorized replacement only. Seven historical local residuals remain protected. Saved layouts, spatial planning, broader restart/Omarchy work, Hermes changes and Phase20 remain separate scope.
...
**Corrected artifact ready, not activated:** `/home/user/.hermes/runs/aiw-hermes-refresh-20260914/web`. Operator45341 still serves accepted media entry `index-BdnukmnR.js`; backend43941/native Edge49341/state/history untouched. Await approval to switch only the TEST frontend. Both disposable45343 probes stopped. Four frontend production files, three tests and this status changed within the inherited68 dirty paths; no staging/delivery or server/media/renderer edits. Full causal findings, attempts, evidence and retest boundary: `/home/user/.hermes/runs/aiw-hermes-refresh-20260914/README.md`.
...
Evidence and launch helpers: `/home/user/.hermes/runs/aiw-claude-restore-20260914/`. Browser `web/`, backend `backend/`, `native-durability.json`, `native-history-reopen.json`, `browser-green-scroll/`, runtime/owners/readiness receipts, and `README.md`. Native Edge main window ownership and enabled state verified after dismissing only its owned sync modal; no sync approval or app clicks. Old state remains preserved. Current task changed six inherited dirty paths plus five newly dirty tracked paths, for66 dirty paths; no staging, commit, push, merge or publication. No accepted visual assets/renderer behavior changed. Later Windows/WSL shutdown, full application restart and broader four-harness acceptance remain unproven, separate criteria.
...
Evidence: `/home/user/.hermes/runs/aiw-scroll-current-20260907/`; reusable diagnostic `tooling/scripts/preview-input-wheel-proof.mjs`.
...
Evidence: `/home/user/.hermes/runs/aiw-workstream-embodiment-20260907/RESUME-OUTCOME.md`, `resume-proof/`, `resume-actor-proof/`, `tests.log`.
...
Primary diagnosis: `/home/user/.hermes/runs/aiw-code-wheel-clean-20260906/DIAGNOSIS.md`; preserved homepage: `state/codex-sessions/index.html` under that run. PR9 receipts: `/home/user/.hermes/runs/aiw-code-wheel-delivery-20260906/MERGED.md`. This docs-only continuation marker is a separate verification era; its SHA/CI outcome belongs in the external handoff, not a recursive repository commit. All dated sections below are historical except explicitly carried-forward defects.
...
**Project-local continuation plan:** [Agent movement continuation](.hermes/plans/2026-09-06_144547-agent-movement-continuation.md). It records exact paths, evidence, recommended mention precedence, focused checks, further movement acceptance and delivery boundaries. Evidence root: `/home/user/.hermes/runs/aiw-movement-20260906/`.
...
The original candidate failed those two visual criteria; its supplied video remains at `C:\Users\User\Videos\flicker screen.mp4` and its retained diagnostic state is historical evidence, not the corrected verdict.
...
Evidence: `/home/user/.hermes/outputs/world-repository-code-2026-09-04-final1/`, with screenshot hashes in `evidence.json`; source run `/tmp/aiw-repository-code-final1/`. Both disposable test ports (45173, 43770) were verified clear. No manual acceptance server remains running. This is technical local proof, not Aaron's manual verdict or full Slice 6 acceptance.
...
`/home/user/.hermes/runs/aiw-phase19-task10-20260813/task10-parent-acceptance.md`.
...
`/home/user/.hermes/runs/aiw-phase19-task9-20260813/task9-parent-acceptance.md`.
...
parent receipt is
`/home/user/.hermes/runs/aiw-phase19-task7-parent-retry-20260813/task7-parent-acceptance.md`.
...
`/home/user/.hermes/runs/aiw-workbench-clarity-retest-20260810T214646Z/candidate/`.
...
`/home/user/.hermes/runs/aiw-workbench-create-retry-20260810T201214Z/candidate/manual-acceptance-20260810-create-inspect-cancel/`.
...
`/home/user/.hermes/runs/aiw-workbench-manual-20260810T183646Z/candidate/failure-20260810-workbench-fetch/`.
...
`/home/user/.hermes/runs/aiw-repository-city-operator-fail-fix-20260806T000741Z/manual-candidate-20260806T005031Z/`.
...
retained at
`/home/user/.hermes/runs/aiw-m2-movement-retest-20260804T134639Z/manual-verdict.json`.
...
`/home/user/.hermes/runs/aiw-chat-manual-retest-20260803T161953Z/manual-verdict.json` and
...
`/home/user/.hermes/runs/aiw-post-message-refresh-retest-20260803T133530Z/manual-verdict-chat-unavailable-state-manifest.json`.
...
`/home/user/.hermes/runs/aiw-post-message-refresh-fix-20260803T000130/manual-verdict-connect-unavailable-v2-state-manifest.json`.
...
`/home/user/.hermes/runs/aiw-refresh-restoration-20260802T175536Z/manual-verdict-post-message-refresh-fail-state-manifest.json`
...
`/home/user/.hermes/runs/aiw-phase18-5-option2-integration-20260802T123857Z/`.
