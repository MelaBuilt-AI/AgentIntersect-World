...
**Next milestone: Phase 16 — Multi-agent coordination, isolated worktrees, and embodied handoffs: DECISIONS FROZEN / AUTHORIZED TO START IN A NEW SESSION / NOT STARTED**

- Phase 15 frozen decision sheet: `docs/PHASE_15_SCOPE.md`; implementation evidence: `PHASE_15_REPORT.md`; exact provider pin and historical artifact proof: `docs/PHASE_15_STT_PROVIDER_PIN.md`, `docs/PHASE_15_STT_ARTIFACT_REPORT.md`, and `artifacts/phase15/stt-provider/`. Phase 16 approved/frozen decision sheet: `docs/PHASE_16_SCOPE.md`.
...
- On 2026-07-23 the user explicitly chose to leave the clean pushed Phase 15 candidate in that state and carry the physical-microphone journey as a pending TODO. The user then froze the bounded Phase 16 contract in `docs/PHASE_16_SCOPE.md` and authorized it to start in a new session with Mr Fluff/Hermes as Agent 1 and same-machine OpenClaw agent `beans` as Agent 2, configured model default `gpt-5.6-sol`, exactly two agents, one private repository, separate worktrees, manual integration approval, and no public services or autonomous swarm. No Phase 16 implementation, branch, worktree, agent, service, dependency, or test activity has started in this session.
...
**Phase 15 — PHYSICAL-MICROPHONE USER ACCEPTANCE PENDING / PROVIDER UNACTIVATED**

`docs/PHASE_15_SCOPE.md` remains frozen. The bounded implementation candidate and independent parent proof are green in `PHASE_15_REPORT.md`, but this host exposes no usable microphone input and the frozen exit gate cannot be truthfully simulated. The next action is one browser-visible physical-microphone push-to-talk journey through disclosure, capture, edit/send, canonical reply, optional TTS, and stop/barge-in, followed by explicit user acceptance.
...
Future runtime/model refetch, provider promotion/activation, release, deployment, public ingress, publication, visibility change, Hermes-core/profile change, original-AgentIntersect operation, Phase 16+, and every action outside the pending physical-microphone acceptance remain prohibited without their required separate authorization.
