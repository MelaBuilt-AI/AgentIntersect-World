...
import { Box3, Group, Mesh, Sprite, type Material } from "three";
import { useCodeTexture } from "./code-world-texture.js";

/** One entrance per mounted World; never remount the avatars or their mixers. */
export function AvatarMaterialization({
  ...
  useFrame((_, delta) => {
    ...
    if (!actors || finished.current) return;
...
`,
  ...
  return (
    <group ref={group} name="world-avatar-materialization" visible={false}>
      {children}
    </group>
  );
}
