...
it("rejects a different identity inside the native Hermes create envelope", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "aiw-hermes-mismatch-"));
  try {
    const adapter = new HermesSessionAdapter({
      baseUrl: "http://127.0.0.1:8642",
      apiKey: "fixture-key",
      ...
      fetch: async () =>
        Response.json(
          { object: "hermes.session", session: { id: "different-session" } },
          { status: 201 },
        ),
    });
    await expect(
      adapter.createWorldSession("world-a", "Mr Fluff"),
    ).rejects.toThrow("different new session identity");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

it.each(["wrapped", "flat"])(
  "creates separate profile-routed Hermes conversations with %s responses and restores exact World ownership",
  async (shape) => {
    ...
    const fetcher = vi.fn<typeof fetch>(async (url, init) => {
      ...
      if (init?.method === "POST") {
        ...
        if ([...sessions.values()].some((s) => s.title === body.title))
          return Response.json(
        ...
        return Response.json(
          shape === "wrapped"
            ? { object: "hermes.session", session: body }
            : body,
          { status: 201 },
        );
      }
    ...
    try {
      ...
      expect(first.title).toBe("Research");
      expect(second.title).toBe("Research");
      expect(sessions.get(first.id)!.title).not.toBe(
        sessions.get(second.id)!.title,
      );
      expect(sessions.get(first.id)!.title.length).toBeLessThanOrEqual(80);
      const restored = new HermesSessionAdapter(options);
      expect(
