...
**User-accepted 2026-08-26:** the normal World reuses Phase 15's explicit-disclosure microphone capture and local Whisper transcription through one adjacent hold control, shows only an editable final caption, and routes accepted text through the exact typed-chat/grouped-message queue. Automated proof already covered pointer, touch, focused-Space, cancellation, device loss, hard ceilings, broadcast, avatar targeting/reset, no raw-audio fan-out, no normal-World speech synthesis, and typed-chat availability; parent proof passed Phase 15 conformance **41/41**, selected World/Phase 19 tests **68/68**, repository typecheck **38/38 tasks**, build **20/20 tasks**, full ESLint, focused Prettier, diff hygiene, Task 14 Playwright **1/1**, and impacted no-WebGL **1/1** under Node `v24.18.0` / pnpm `11.15.0`. A fresh receipt-owned Edge lane then used the production web bundle, production VoiceService, real staged local `whisper.cpp`, and disposable agent/message state. Aaron reported `PASS 1-9` for disclosure-before-permission, explicit enable, pointer Cancel/no-send, focused-Space edited four-agent broadcast, avatar-selected edited Codex-only target, target reset, typed-chat availability, desktop/mobile containment, and physical device-loss discard. Eleven capture requests produced eleven redacted operation records (seven completed, four failed/device-loss attempts), grouped payloads contained no audio fields, final voice activity was idle, and volatile audio was empty. One extra typed `hi` broadcast is retained as operator-input evidence, not a defect. Receipt: `/home/user/.hermes/runs/aiw-phase19-task14-manual-20260826T065547-0400/manual-acceptance-verdict.json`. Aaron separately authorized private Task 14 commit/push on 2026-08-26. The first pushed checkpoint's exact-SHA CI exposed an unused-voice teardown mutation: ordinary reset/detach could send `activity: null` and produce fixture 404s without any prior voice activation. The existing reset/detach journey reproduced the exact RED locally. The direct correction tracks activity ownership from successful capture initiation and clears it only for an owned capture; the causal reset journey, all three formerly red shard-3 journeys, the Task 14 voice journey, focused voice tests/typecheck/lint/build, and the shard-4 returning-user journey pass locally. The replacement run cleared every Task 14 lifecycle failure and shard 4. Flagged shard 3 attempt 1 timed out only at an unchanged final evidence screenshot; attempt 2 passed tests 1–18 and was cancelled by the job's 20-minute ceiling while test 19 was still running. The exact unchanged animation journey passed in a fresh bounded local two-CPU/8-GiB service in **2m24s** with **1.3 GiB** peak and no swap. A workflow-only RED→GREEN correction therefore raises only the flagged E2E job ceiling from 20 to 30 minutes while preserving the four-shard topology, product behavior, assertions, test-local timeouts, retries, and fail-closed gates. Final replacement SHA/CI evidence remains external rather than recursively committed. Task 15 and later remain closed; no protected-runtime mutation, PR/merge, release, or publication action is authorized.

### Task 15 — Browser acceptance and real-harness proof

**Status:** **USER-ACCEPTED 2026-08-28; PRIVATELY DELIVERED ON SOLE EXACT-GREEN `main`.** Retest-6 evidence led to two causal corrections: dynamic `current` repository sessions now resolve against the selected concrete repository for server-owned structured movement, and durable grouped results now list, restore, and incrementally update stable recipient rows. Retest 7 first-hand passed Codex/Claude exact-target movement and headings, visible post-refresh replies, Repository City persistence, target clearing, same-World/four-agent refresh, and prior voice criteria. Its only follow-up was a post-arrival imported-avatar T-pose. Exact raw-clip review for the active `cat-agent-02` and `cat-agent-03` models found no unambiguous Dig/coding/examine clip, so no semantic was guessed. Imported `Work` now uses each model's already approved animated Idle clip as the truthful Work/static fallback. In fresh retest 8 Aaron corrected his verdict to **Codex PASS and Claude PASS** with no T-pose. The authoritative verdict and accepted pixels/state are under `/home/user/.hermes/runs/aiw-phase19-task15-retest8-20260828T201604-0400/`. Final private delivery is `b0b58bd18e4c3fd784b39e8c8a344125c9e2e6a0`; Dig remains unmapped and non-blocking for Phase 19 closure. Task 16 is now authorized; Animation Confirmations, Phase 20, provider/profile changes, and public/release actions retain separate gates.
