...
**Status:** **COMPLETE — TASK 16 CLOSED 2026-08-30.** Aaron first-hand accepted Edge/RTX behavior plus distinct duplicate-Codex sessions; `PHASE_19_REPORT.md` maps all 19 criteria; the focused scheduler correction and affected browser behavior are green. The 76/77 fresh-copy software-rendering budget result is non-blocking diagnostic history. Dig belongs to the next separately gated Animation Confirmations milestone.
...
6. Real structured tool/work evidence may resolve an agent's current code target to the most specific live Repository City object, then move that agent through the existing generation-bound `repository-object` contract to its safe approach point and face/interact there. After truthful arrival and while coding remains active, imported avatars use each exact model's approved animated Idle clip as accepted Phase 19 Work/static truth. Assistant prose never implies work. Unresolved, stale, or blocked targets remain visible and recoverable; completion, failure, cancellation, or retargeting clears coding presentation deterministically. Dig is separately gated after Phase 19 closure.
...
**Tests/evidence:** Task 16 reused accepted first-hand local proof for every adapter and the combined four-agent journey, then mapped deterministic, browser, recovery, privacy, Phase 18, and Phase 16 evidence in `PHASE_19_REPORT.md`. Exact-SHA CI covers 1048 passing core tests plus one intentional skip, Phase 16 41/41, 76/76 flagged browser tests, and 2/2 unflagged fail-closed tests. The focused scheduler correction passed 21/21 owning tests, format, typecheck, build, and affected browser behavior with post-fix Phase 18.5 render-work p95 1.0 ms and longest task 85 ms. The later 76/77 fresh-copy software-rendering budget result is non-blocking diagnostic history. Phase 19 coding truth remains repository-object movement plus exact model-local animated Idle Work/static; Dig evidence moves to Animation Confirmations.
...
**Stop condition now:** Phases 0–17 remain accepted historical foundations. Phase 13 Discord → World continuity remains FAIL/deferred under waiver and was not retried. Phase 15’s local STT provider remains staged/unactivated. Revised Phase 18 is complete and accepted. Phase 19 Tasks 1–16 are complete; Tasks 13–15 remain user-accepted and privately delivered on sole `main` at `b0b58bd18e4c3fd784b39e8c8a344125c9e2e6a0`. The next milestone is Animation Confirmations for all 23 models, including unresolved model-local Dig. Phase 20 begins only after animation integration and acceptance. Git delivery, provider/profile changes, and release/public actions retain separate authorization gates.
