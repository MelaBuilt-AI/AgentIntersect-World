...
- The latest user-ordered sequence is canonical in `docs/PRE_VOICE_ANIMATION_MOVEMENT_AND_CODE_VISUALS.md`: before voice or bounded live coding, correct Space/Jump and all eight local slash-command animation semantics, complete autonomous agent movement with correct locomotion/gesture animation, then ingest and integrate the 26 user-labeled code/repository/action visual assets. Do not infer animation semantics, claim a task accepted before first-hand visual proof, waive/seal Phase 18 or Phase 18.5, or begin Phase 19/20 without the corresponding later user acceptance/authorization.

## 10. Phase 19 authorized scope

- Aaron authorized Phase 19 on 2026-08-11. Tasks 1–12 are parent-accepted, and Tasks 13–14 are **USER-ACCEPTED** (Task 13 on 2026-08-24; Task 14 on 2026-08-26). Task 15 and later remain closed. Aaron selected exactly `1A 2A 3A 4A 5A`; the stable four-slot registry, independent sanitized readiness/capability truth, deterministic conformance matrix, direct World-owned gateway/API lifecycle, production wiring, narrow strict safe-config alignment, harmless production attestation, durable constellation service/routes, bounded ordered-entry reducer/client, visible four-agent constellation/rendering, durable server-owned grouped chat/targeting, exact independent repository-object work focus, recovery/migration composition, and normal-World push-to-talk are green. No Dig mapping was operator-approved, so Task 12 retains truthful Work/static coding fallback. Task 13 restores accepted active constellations without duplicate native identities and preserves all five actors through refresh and Repository City load. Task 14 passed real Edge microphone → local Whisper → editable caption → four-agent broadcast and Codex-only targeting, Cancel/no-send, focused-Space, automatic target reset, typed-chat availability, desktop/mobile containment, physical device-loss discard, no raw-audio fan-out, and no normal-World agent TTS. Receipt: `/home/mela_ai/.hermes/runs/aiw-phase19-task14-manual-20260826T065547-0400/manual-acceptance-verdict.json`. Aaron separately authorized private Task 14 commit/push on 2026-08-26; final SHA/CI evidence remains external. Task 15+ remains closed.
