...
- The latest user-ordered sequence is **Phase 19 Task 16 closure → Animation Confirmations for all 23 models, including model-local Dig → Phase 20**. The Phase 19 plan, `PROJECT_STATUS.md`, and `docs/WORLD_ENTRY_EXPERIENCE.md` are the current authority. Older pre-voice/animation ordering remains historical context and must not override this sequence. Do not infer animation semantics, claim a gate accepted before first-hand visual proof, or begin Phase 20 before Animation Confirmations are integrated and accepted.

## 10. Phase 19 authorized scope

- Aaron authorized Phase 19 on 2026-08-11. Tasks 1–12 are parent-accepted, Tasks 13–15 are **USER-ACCEPTED** and privately delivered on sole exact-green `main` at `b0b58bd18e4c3fd784b39e8c8a344125c9e2e6a0`, and Task 16/Phase 19 are complete under the 2026-08-30 functionality-first closeout. Task 15 passed first-hand four-agent refresh, exact targeting/reset, durable visible grouped replies, Repository City persistence, real Codex/Claude exact-object Walk/Run → arrival, correct final headings, voice carry-forward, and a focused post-arrival no-T-pose retest. Imported `Work` intentionally uses each exact model's approved animated Idle clip as accepted Phase 19 Work/static truth. Dig remains fail-closed until Animation Confirmations. Mr Fluff codes directly; do not use Codex/delegated coding unless Aaron explicitly requests it. Animation Confirmations implementation, Phase 20, PR/merge, provider/profile changes, and all release/public actions retain separate gates.
...
- Only real structured tool/work evidence may resolve an agent's current code target to the most specific live Repository City object and drive the existing generation-bound repository-object movement contract to its safe approach point. After truthful arrival and while coding remains active, use each exact model's accepted animated Idle clip for Phase 19 Work/static. Never infer work from assistant prose. Keep unresolved, stale, or blocked targets visible/recoverable; clear coding presentation deterministically on completion, failure, cancellation, or retargeting; keep agents independent. Dig remains fail-closed until separate model-local Animation Confirmations.
...
- After closure, extend the 23-model offline review authority with unresolved model-local Dig, growing 207 decisions to 230 without preselection; preserve all 417 raw clips and 69 locked locomotion decisions.
- Animation reports are semantic-review authority, not runtime activation. Generate runtime mappings separately, fail closed for uncertain/unsupported decisions, and obtain normal-product first-hand acceptance.
- Begin Phase 20 only after Animation Confirmations are integrated and accepted. No commit/push, provider/profile mutation, public/release action, or Phase 20 work is implied by Task 16 authorization.
